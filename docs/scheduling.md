...
3. **The unit was removed outside contextlake.** `status` reports "recorded but NOT installed"
   ...
   `contextlake schedule install`.
4. **`schedule_require_idle` is on and you expected it to gate a run.** It cannot: see
   [Platform differences](#platform-differences).
5. **Consecutive failures are backing off the interval.** `status` reports the failure count. Fix
   the underlying command failure (run it by hand first) and the next success resets it to zero.
6. **Nothing has fired yet at all.** Check the platform directly: `systemctl --user list-timers`
