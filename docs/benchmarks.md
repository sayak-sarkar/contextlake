...
Any session pays for the tool schemas once, whether or not it calls a tool: **21 of them on a
graph-only store** (20 graph tools plus the `ask` router) and **23 once embeddings exist**, all
