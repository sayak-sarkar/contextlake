...
Any session pays for the tool schemas once, whether or not it calls a tool: **20 of them on a
graph-only store** (19 unconditional plus the `ask` router) and **22 once embeddings exist**, all
