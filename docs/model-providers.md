...
Generating one wiki page is not one model call. It is the page, plus one review from each
council lens. A rejected page is reported and skipped, not rewritten, so a rejection costs you
the page rather than another round of calls.

So the real question is not whether a 1.5B to 3B model writes better prose than the built-in
0.5B. It does. The question is whether your machine can finish that many calls.

- **On a CPU with no GPU**, a single call from a 1.5B to 3B model usually takes minutes. The run
  hits the default 300-second `timeout` below before a page is finished.
- **The built-in 0.5B stays under that timeout.** It pays for that with thin, generic prose the
  council often rejects.
- **With a GPU, the same 1.5B to 3B models answer in seconds.** That includes Ollama on a
  Windows host with a discrete GPU, reached from WSL.
