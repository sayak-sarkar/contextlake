...
- **Every test in a C++ repo was invisible by name.** A test macro with a body parses as a function
  definition whose name is the *macro*, so `TEST(DateTimeCompare, Minutes)` produced one node called
  ...
  or `TEST_P` (3). Asking "where is the OrderCreate invalid-offer test" had no answer, and because
  ...
  So `TEST(DateTimeCompare, Minutes)` is a `test` named `Minutes` qualified
  `DateTimeCompare.Minutes`, and two suites that share a case name stay two distinct nodes.
