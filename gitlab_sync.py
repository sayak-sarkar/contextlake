#!/usr/bin/env python3
"""Bare-script launcher for gitlab_sync.

Lets the tool run without installation via ``python3 gitlab_sync.py <command>``.
It puts the ``src/`` layout on ``sys.path`` (ahead of the cwd) so that
``import gitlab_sync`` resolves to the package under ``src/gitlab_sync`` rather
...
# `import gitlab_sync` resolves to the package, not to this same-named launcher.
...
from gitlab_sync.cli import main  # noqa: E402
